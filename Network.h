...
class network {
	public:
		network(const vector<int>& nNeurons, const int nInputs); 	// Constructor1, when no weights or biases are given. Paramters are # neurons for every layer, #inputs for the first layer of neurons, # layers.
		...
		vector<vector<float>> getLayerResult();			// get-function that returns the output vector of each layer

		vector<vector<vector<float>>> loadLayers(const string); // Loads weigths and biases from a csv file 
		void saveLayers(const string); 				// Saves the current values of all the biases and weights in the network to a file
